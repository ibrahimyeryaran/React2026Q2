...
export const App = () => {
  ...
  // Deriving the available years scans every country/year, so memoize on data.
  const years = useMemo(() => (data ? getAvailableYears(data) : []), [data]);
  // The column catalog is static; compute once so the reference stays stable.
  const availableColumns = useMemo(() => getAvailableColumns(), []);

  // Functional updates let every handler be referentially stable (empty deps),
  // which keeps memoized children from re-rendering on unrelated state changes.
